...
Five properties are load-bearing for the whole system. They are stated **here, once**; later sections reference this list rather than restating it.
...
5. **Hard-validate semantic shape only; mechanical bounds degrade gracefully.** An LLM-output schema hard-validates only whether the answer is _meaningful_ — the answer code is one of the five `LLMAnswerCode`s, and the quote/justification fields are present as strings. Mechanical bounds (output length, formatting niceties) are **not** enforced at the validation boundary, because a failure there is destructive: it routes through the [REQ-06](#req-06-llm-abstraction-layer) repair ladder and can collapse a substantive answer into a format-failure `NI` ([REQ-20](#req-20-error-handling-and-retry)/[REQ-21](#req-21-evaluation-harness)). Mechanical bounds are applied deterministically in `finalize_sq_answer` ([REQ-15](#req-15-sq-worker)) instead, where over-length degrades to a recorded soft-truncation rather than a failure. The test for "may a violation fail the call" is whether the violation means _the answer is not meaningful_ — which is why excluding `NA` from `LLMAnswerCode` ([§5.3](#53-trial--outcome-models)) is a legitimate hard-validation (an LLM-emitted `NA` is _always_ wrong, NA being structural) while a 30-character-too-long quote is not. This keeps the substantive-vs-format-failure `NI` split ([REQ-21](#req-21-evaluation-harness)) measuring what it is meant to.
...
Assessment runs as two LangGraph graphs (trial tier and outcome tier; [REQ-12](#req-12-assessment-orchestration-two-tier-graph)). LangGraph is chosen for its **checkpoint/resume runway** toward the living-evidence deployment: v0.1 runs in-memory with **no checkpointer**, but the node/state structure is the seam a later resume layer plugs into. For that seam to be real, **state holds only serializable data**: runtime handles (LLM clients, the retrieval index, the trace collector) are passed via the graph's **runtime context** (LangGraph `context_schema` → `AssessmentRuntime`, [§5.6](#56-langgraph-state)), read in nodes as `runtime.context.<handle>`, and so sit **outside** the checkpoint boundary by construction — never in state. This single rule follows from invariant 3, and it is what lets the v0.2 checkpointer attach without first tearing handles out of state.
...
The principal saving is **ingestion reuse** (parse, supplement index, CT.gov, metadata), which dominates per-trial cost; sharing D1 saves only its three SQ calls. D1 is shared because randomisation happens once. D2 is **not** shared: its SQs (e.g. 2.4/2.5) reference "the outcome," and a deviation can affect one outcome and not another, so D2 is judged per outcome alongside D3–D5. D2's _blinding_ inputs (2.1/2.2) are themselves trial-invariant, but D2 is kept **whole** in the outcome tier rather than hoisting 2.1/2.2 into the trial tier: that split would save only a couple of SQ calls while breaking "one domain = one tier," threading partial-D2 state across tiers, and complicating 2.3's gate (which references 2.1/2.2) — so 2.1/2.2 are deliberately re-evaluated per outcome for tier uniformity. At temperature 0 over the byte-identical shared prefix, their answers are stable across a trial's outcomes, so the redundancy costs a little and risks no incoherence.
...
>
> **Not in v0.1 (planned; interfaces kept extension-ready):**
>
> - the **CONSORT vision vertical** — `prompts/consort_vision.py`, the trial-tier `graph/nodes/consort_extract.py`, the figure detector, and `ChatOpenRouter.complete_vision` — deferred to v0.2 (§3); v0.1 keeps only the seam (inventory in [§3](#3-users-and-workflow));
> - graph-level **checkpoint/resume**, and **refresh-on-change** re-assessment for the living-evidence deployment ([REQ-17](#req-17-batch-runner-and-manifest));
> - a cross-encoder **reranker** over the hybrid-fused candidates ([REQ-03](#req-03-supplementary-material-ingestor));
> - a prompt-hash **LLM response cache**.
>
> **Artifact-reuse caching is deliberately rejected** — its stale-artifact bug class fights the "see the real data" goal; intermediates are dumped for inspection, not reused ([REQ-23](#req-23-run-trace--timing-instrumentation)).
...
class TrialMetadata(BaseModel):
    ...
    intervention: str
    comparator: str
    all_outcomes: list[str]        # primary first, then secondary; capped at ARBITER_MAX_OUTCOMES (default 10). RECORD METADATA / manifest-authoring aid only — NOT a work list: only the manifest-enumerated `outcomes` (else the primary) are actually assessed (REQ-17). The cap bounds the extracted list, not how many outcomes are judged.
...
# The five codes the LLM is ALLOWED to emit. `NA` is deliberately absent: NA is
# STRUCTURAL — set only by branching (REQ-08), never generated — so excluding it from the
# SQ schema makes "the LLM cannot emit NA" (§1/REQ-09) true at the validation boundary, not
# just by prompt instruction. A stray `NA` from the model then fails schema validation and
# routes through the REQ-06 repair ladder (→ recorded format-failure NI/FLAGGED if it
# persists, REQ-20) instead of being silently accepted as a confident structural NA that
# would drop a real SQ out of the deterministic domain judgment.
LLMAnswerCode = Literal["Y", "PY", "PN", "N", "NI"]   # ⊂ AnswerCode, minus NA
...
class SQRawAnswer(BaseModel):
    ...
    are the quote, the answer code, and the justification. `answer` is the
    restricted `LLMAnswerCode` (no `NA`); the full `AnswerCode` (with `NA`) is the
    type of the STRUCTURAL `SQAnswer.answer` that branching populates.

    Per invariant 5 (§2), this schema hard-validates only SEMANTIC SHAPE — the
    answer code and the presence of the quote/justification strings. It carries
    NO tuning-relevant length cap: a tight `max_length` enforced here is
    destructive (it routes a too-long-but-substantive answer through the REQ-06
    repair ladder and can record it as a format-failure NI). Output length is
    bounded GRACEFULLY in `finalize_sq_answer` (REQ-15) — over-length is
    soft-truncated for storage with the original length recorded in the trace,
    never failed. The `max_length`s below are only GENEROUS ANTI-ABUSE CEILINGS
    (a model emitting a whole page is misbehaving); they sit far above any
    genuine 1-2 sentence quote, so they never trip a real answer."""
    answer: LLMAnswerCode
    quote: str = Field(max_length=4000)          # anti-abuse ceiling, not a tuning cap (invariant 5)
    justification: str = Field(max_length=1000)  # anti-abuse ceiling, not a tuning cap (invariant 5)
...
class SkipRecord(BaseModel):
    ...
    created_at: str                # ISO 8601
    pipeline_version: str
    model_sq: str                  # the configured SQ model — a unique-key column (REQ-16); carried so the skip row's key is non-NULL and the row UPSERTs across re-runs
...
A `SkipRecord` is written as **one SQLite row per trial** with sentinel **unique-key** columns — `outcome = "__TRIAL__"` (the trial has no eligible outcome to scope) and `effect_of_interest = "__NA__"` (ineligibility is effect-independent) — `model_sq`/`pipeline_version` carried from the record, and `NULL` in every judgment column (see [REQ-16](#req-16-output-json-and-sqlite-writers)), plus a `skip.json` at `{output_dir}/{trial_id}/skip.json` for audit parity. The key columns are **non-NULL sentinels, never `NULL`**, because SQLite treats `NULL`s in a UNIQUE index as distinct — `NULL` key columns would make every re-run append a fresh skip row instead of upserting. No per-outcome rows, no `data.json`.
...
Two TypedDicts — one per tier — plus the runtime-context dataclass. Per §2 (Orchestration + invariant 3): no LLM client, retrieval index, or `RunTrace` collector is stored in state at all — they are passed via the graph's **runtime context** (LangGraph `context_schema=AssessmentRuntime`, below) and read in nodes as `runtime.context.<handle>`, so the state stays pure serializable data and the v0.2 checkpoint boundary never tries to serialize a non-serializable handle. No trace/timing data appears in the reducers or the `Assessment` record.
...
# D2–D5 accumulation), so they are declared per tier. State holds ONLY serializable data —
# every runtime handle (LLM clients, supplement index, trace) lives in the graph's runtime
# context (`AssessmentRuntime`, below), never in state, so state is a clean checkpoint seam
# for the v0.2 resume layer (invariant 3, §2).
...
# The vision CLIENT is a runtime handle and lives on `AssessmentRuntime` (below), not state;
# the extracted `consort` (pure data) is the only CONSORT field on state, and on TrialState
# ONLY — not the shared base — so the outcome tier carries no always-None `consort` slot in v0.1.
...
class TrialState(_IngestionState):     # trial tier: D1 (+ v0.2 CONSORT extraction)
    ...
    consort: ConsortExtraction | None      # detection + extracted flow as one object (once per trial); KEPT SEAM — always None in v0.1, populated by the v0.2 vision vertical (pure data, so it stays in state; the vision client is a runtime handle on AssessmentRuntime)
...
class DomainContext(BaseModel):
    ...
    retrieval_top_score: float | None = None  # ABSOLUTE [0,1] relevance magnitude of the RRF-top passage — its dense cosine to the query (NOT the raw RRF score, NOT a min-max value); None when there is no dense signal (BM25-only arm / empty index). Full definition: REQ-03/REQ-11.
    segments_retrieved: int = 0
    segments_available: int = 0

# The graph's RUNTIME CONTEXT (LangGraph `context_schema`): every non-serializable runtime
# handle the nodes need, passed via `graph.invoke(state, context=AssessmentRuntime(...))` and
# read in nodes/edges as `runtime.context.<handle>` (the node signature gains a
# `runtime: Runtime[AssessmentRuntime]` param). It is NOT state and NOT checkpointed, so
# attaching a checkpointer in v0.2 never tries to serialize a client/index/trace — this is the
# clean checkpoint seam invariant 3 (§2) promises. `build_trial_graph`/`build_outcome_graph`
# compile with `context_schema=AssessmentRuntime` (REQ-12). `assess_trial` builds one from the
# handle fields of `TrialContext` (below) while seeding that context's DATA fields into state.
@dataclass
class AssessmentRuntime:
    ...
    llm_client_vision: LLMClient | None = None   # KEPT SEAM — None in v0.1 (no vision)
...
- **Orchestration:** `langgraph` (**≥ 0.6** — the run requires the `context_schema` / `Runtime` runtime-context API used to pass handles outside state, [§5.6](#56-langgraph-state)/[REQ-12](#req-12-assessment-orchestration-two-tier-graph)), `langchain-core` _(no `langgraph-checkpoint-sqlite` — v0.1 does not checkpoint)_
...
**Every operational threshold/cap/budget below is an env knob with a conservative default — no behaviour-governing magic numbers are hard-coded.** The defaults are **set a priori** for accuracy-first, free-tier-cheap operation and **frozen** — they are exposed so the [REQ-21](#req-21-evaluation-harness) ablations can **sweep them as sensitivity analyses** rather than a code edit, **not** so a sweep can pick the shipped value (no operational threshold is selected by maximising accuracy on any reported set — the develop/report-separation rule in [REQ-21](#req-21-evaluation-harness)). (Two numeric constants are deliberately **not** env knobs because they are _policy/contract_, not operational tuning: `OVERALL_HIGH_SC_THRESHOLD` — owned by an ADR + bumps `pipeline_version`, [REQ-07](#req-07-rob-2-algorithm-deterministic-judgments) — and the `SQRawAnswer` `quote`/`justification` **anti-abuse `max_length` ceilings**, which are generous data-contract guards, not tuning knobs; the substantive length policy is graceful soft-truncation in `finalize_sq_answer` per invariant 5 (§2), [§5.3](#53-trial--outcome-models).)
...
`SupplementIndex.retrieve(query_terms: list[str], domain: str, top_k: int = 5) -> tuple[list[SupplementSegment], float | None]` returns the top-k segments (ranked by **RRF fusion** of the BM25 and dense lists) and an **absolute [0,1] relevance magnitude** for the top segment — the **dense cosine similarity** of the RRF-top passage to the query, clamped to [0,1]. It is deliberately **not** the raw RRF value (a rank artefact) **and not a min-max normalisation over the candidate set**: min-max would pin the top passage to ~1.0 by construction (it is the argmax of at least one retriever), making the [REQ-11](#req-11-confidence-signal-system) `UNCERTAIN`/`FLAGGED` score thresholds **dead code**. A dense cosine is an absolute scale comparable across queries, so the 0.35 floor is meaningful. Returns `None` for that score when there is **no dense signal** — the **BM25-only baseline arm** or an empty index — in which case REQ-11's score-based clauses correctly do not fire. It filters to segments tagged with `domain` first; if fewer than 2 such segments exist, it falls back to the full set. RRF stays the **ranking** mechanism; only this surfaced confidence magnitude is the absolute dense cosine. The index is in-memory only; **never serialised to disk**.
...
**`locate_quote_page`** — the deterministic page resolver that replaces the LLM-supplied page ([REQ-15](#req-15-sq-worker)/[§5.3](#53-trial--outcome-models)). Normalise the quote the same way and `partial_ratio`-match it against **per-page concatenated text** — group `page_boxes` by `page`, join each page's boxes in reading order, and match the whole quote against each page's text. Return the `page` of the best-scoring page above threshold (the **earliest** page on a page-break straddle, by also scoring each page joined with the next and attributing the match to the earlier page). Match **per page, not per box**: a quote that spans several boxes on one page verifies against `raw_char_stream` but no single box would clear the threshold, so per-box matching would wrongly return `None` — per-page concatenation puts the whole quote in one matching unit. Returns `None` for an empty/sub-`ARBITER_QUOTE_MIN_VERIFY_CHARS` quote (NA/NI), or when no page clears threshold. Matching against `page_boxes` — which carry per-page text + `page` — recovers the page from the layout parse without threading page offsets through the flat `raw_char_stream`.

**`resolve_quote`** — the **facade** the SQ worker calls ([REQ-15](#req-15-sq-worker)); it normalises the quote **once** and runs verification + page resolution together, returning `(verified, page)`. It owns the invariant `locate_quote_page` cannot (it has no verification result): (1) `verify_quote` the quote against `raw_char_stream`; (2) if **not** verified → return `(False, None)` (an unverifiable quote never gets a page — this is what makes `page is None iff no verifiable quote` true, [REQ-15](#req-15-sq-worker)); (3) if verified → `locate_quote_page`, and on the rare verified-but-unlocalised quote (genuinely split across the layout such that no page clears threshold) fall back to the **best-scoring page anyway** so a verified quote always resolves to _a_ page — a marginally-wrong page beats a null one, and the quote is already known to be in the document. This single facade is also where the double-normalisation of the old separate `verify_quote`/`locate_quote_page` calls is removed.
...
- `locate_quote_page`: returns the correct 0-based page for a quote on a known page of a fixture; returns the **earlier** page for a quote straddling a page break; **resolves a quote split across two boxes on one page to that page** (per-page matching, not per-box); returns `None` for an empty/NA/NI quote and for a quote absent from the document — with no LLM or network call.
- `resolve_quote`: returns `(False, None)` for an unverifiable quote; `(True, page)` for a verified quote, including a non-null **best-page fallback** for a verified quote that no single page clears at threshold; normalises the quote once (no double pass) — with no LLM or network call.
...
> **Score scale (read first).** `retrieval_top_score` is an **absolute [0,1]** relevance magnitude — the **dense cosine** of the RRF-top passage to the query (full definition in [REQ-03](#req-03-supplementary-material-ingestor)/[§5.6](#56-langgraph-state)) — **not** the raw RRF fusion score and **not** a min-max value, so `ARBITER_RETRIEVAL_UNCERTAIN_THRESHOLD` (default **0.35**) is anchored to the cosine scale and is genuinely meaningful (a min-max value would sit at ~1.0 and the threshold would never bite). It is `None` when there is **no dense signal** — the **BM25-only baseline arm**, an empty index, or no domain key-terms; the two score-based clauses below treat `None` as **"no score signal" — they do not fire** (a missing score never triggers `UNCERTAIN`/`FLAGGED` on its own), so the comparison can never throw.
...
> **NI over-selection is a measured concern, not just a flagged one.** The `FLAGGED`/`UNCERTAIN` rules above _detect_ a suspicious `NI`; the _prevention_ is the PY/PN-bridge wording in [REQ-09](#req-09-signaling-question-prompt-templates). The eval ([REQ-21](#req-21-evaluation-harness)) reports ARBITER's per-domain **NI rate vs the human NI rate**; if ARBITER runs persistently NI-heavy, that is the eval-gated trigger to enable **targeted re-retrieval** (widened context for `FLAGGED`/`NI` SQs — deferred below). `retrieval_top_score` is the **absolute dense-cosine** relevance of the top passage (above). On the **hybrid default** (the shipped retriever) it is always present, so the score-based clauses are live; on the **BM25-only baseline arm** it is `None` and those two clauses do not fire — the BM25-only arm then leans on quote-verification and the no-supplements-`NI` clause for its flags. This is the one place flag behaviour differs by retriever, called out so the eval reads it correctly rather than assuming a single score scale across arms.
...
Two LangGraph graphs reflecting the trial/outcome tiers. **No checkpointer** — graphs run in-memory; resilience is at the batch layer ([REQ-17](#req-17-batch-runner-and-manifest)). Both are compiled with **`context_schema=AssessmentRuntime`** ([§5.6](#56-langgraph-state)): every runtime handle (LLM clients, supplement index, trace) is supplied at invoke time via `context=`, read in nodes as `runtime.context.<handle>`, and **never stored in state** — so state stays serializable and the v0.2 checkpointer attaches cleanly.
...
**Why parallel, not serial.** The conditional chains serialise _within_ a domain (e.g. D2 assignment `2.1 → 2.3 → 2.4 → 2.5`, ~4 sequential LLM round-trips; D3 and D4 likewise depth-4, D5 only 1–2 per [REQ-08](#req-08-conditional-branching)), so four domains in series would be ~14–16 sequential round-trips per outcome; as parallel branches it is ≈ max(4,4,4,2) ≈ 4 (the deterministic `pre_d5`/`context` nodes add no LLM round-trip). The rate-limit guard is already in place: `ARBITER_MAX_CONCURRENCY` is a **single global semaphore over LLM calls** ([REQ-17](#req-17-batch-runner-and-manifest)), so overlapping the domain branches cannot cause a 429 storm — it only lets calls that would happen anyway run concurrently. (The win is single-outcome latency on the `assess`/dev path; at batch scale the global semaphore is already saturated by cross-trial parallelism, so throughput is roughly unchanged.)
...
`sq_answers` and `domain_contexts` use a dict-merge reducer (never overwrite). `domain_judgments` and `errors` use `operator.add`. All other fields are last-write-wins. Runtime handles (LLM clients, retrieval index, trace) are **not in state at all** — they are passed via the graph's runtime context (`context_schema=AssessmentRuntime`, [§5.6](#56-langgraph-state)) and read as `runtime.context.<handle>`, so the reducers and the checkpoint boundary only ever see serializable data.
...
**Each node is wrapped at build time** in a span decorator that, at invocation, reads the trace handle from the runtime context (`runtime.context.trace`; `None` → the wrapper is a no-op) and opens a `contextvars`-scoped `NodeSpan` (`tier`, node name, and — outcome tier — `outcome`) around the node body, recording its wall-clock duration and any raised error. The builder therefore takes **no** `trace` argument — the handle lives on `AssessmentRuntime` ([§5.6](#56-langgraph-state)) like every other runtime handle. Node bodies are **not edited** — the wrapping happens in the builder. Because the span is a contextvar, it propagates across LangGraph parallel `Send` fan-out (each SQ worker task copies the context and times itself), and the LLM `CallRecord`s ([REQ-06](#req-06-llm-abstraction-layer)) attach to whichever span is active. The collector is an ephemeral runtime handle (invariant 3, §2).
...
- **Budget:** the cacheable prefix (`shared_prefix_text` **incl.** `ct_gov_block`) ≤ `ARBITER_PREFIX_TOKEN_BUDGET` (default 4,000) tokens; the supplement block ≤ `ARBITER_SUPPLEMENT_TOKEN_BUDGET` (default 2,000) tokens; `domain_specific_text` ≤ `ARBITER_DOMAIN_TEXT_TOKEN_BUDGET` (default 1,500) tokens; plus the SQ's fixed question/definitions (small, template-bounded). **Every term is bounded**, so the worst case is **≤ ~7.5k** and the typical total **~6k** (the prefix is ~4k; most domains' extra sections are well under their 1.5k cap). ≈6k is the **typical total**, never the prefix. All budgets are conservative defaults, exposed as knobs precisely so the [REQ-21](#req-21-evaluation-harness) recall@k / per-SQ ablation can move them as **sensitivity analyses** rather than a code edit (the discipline stays "narrow context per SQ"; per the develop/report-separation rule, the eval characterises but does not _select_ the shipped budget — [REQ-21](#req-21-evaluation-harness)).
...
- Every context term is independently bounded (prefix ≤4k, supplement ≤2k, `domain_specific_text` ≤~1.5k); combined main+supplement context is **~6k typical, ≤ ~7.5k worst case** — never unbounded.
...
- **Finalize the raw answer through one deep function** — `finalize_sq_answer(raw: SQRawAnswer, sq_id: str, context: DomainContext, *, raw_char_stream: str, page_boxes: list[PageBox]) -> SQAnswer` — which hides the three coupled deterministic post-LLM steps and **owns the invariants that bind them**, so the SQ node never re-derives their ordering or short-circuits by hand:
  1. **Resolve the quote** via `resolve_quote(quote, raw_char_stream, page_boxes)` ([REQ-10](#req-10-quote-verifier)) — one normalised pass that verifies against `raw_char_stream` **and** derives the page deterministically (per-page match + verified-best-page fallback), returning `(verified, page)`. The LLM never supplies the page.
  2. **Bound output length gracefully** (invariant 5, §2) — if the quote/justification exceed their soft length, **soft-truncate for storage** and record the original length in the trace; never fail. Verification in step 1 runs on the model's actual returned quote, before any truncation.
  3. **Compute** confidence ([REQ-11](#req-11-confidence-signal-system)) from the verification result + the `context` retrieval signals. **No second sample.**
     The invariants enforced **once, here**: an `NI` answer short-circuits to empty quote + `page = None` (the LLM cannot emit `NA` — it is excluded from `SQRawAnswer`, [§5.3](#53-trial--outcome-models) — so `finalize_sq_answer` never receives one; structural `NA` `SQAnswer`s are built by branching, [REQ-08](#req-08-conditional-branching), and never pass through here); `page is None` **iff** there is no verifiable quote (the `resolve_quote` contract); `compute_confidence` consumes step 1's verification boolean. `verify_quote`, `locate_quote_page`, `resolve_quote`, and `compute_confidence` remain **separately unit-tested pure functions** ([REQ-22](#req-22-testing)) — `finalize_sq_answer` is a thin facade over them, not a merge, and is the seam the v0.2 CONSORT/vision path reuses to emit an `SQAnswer` without copying this logic.
...
The judgment columns (`overall_judgment`, `d1..d5_judgment`, `flagged_sq_count`, `uncertain_sq_count`) are **nullable**, because an ineligible-trial **`SkipRecord`** ([§5.4](#54-the-assessment-record-flat-one-per-trial-outcome)/[REQ-17](#req-17-batch-runner-and-manifest)) writes **one row per trial** with `study_design` set, judgment columns `NULL`, `requires_human_review = 1`, and the reason in `errors` — no `data.json`, no per-outcome rows. **Its five unique-key columns are all non-NULL sentinels**: `outcome = "__TRIAL__"`, `effect_of_interest = "__NA__"`, `model_sq`/`pipeline_version` from the record. This is mandatory, not cosmetic: SQLite treats `NULL`s in a UNIQUE index as **distinct**, so a skip row with `NULL` in any key column would never trigger `ON CONFLICT` and every re-run would append a duplicate skip row instead of upserting the one. On a skip row `title` is `NULL` (the minimal `SkipRecord` does not carry it) and `json_path` points at the trial's `skip.json`, not a `data.json`.
...
- **Pre-ingestion entry skip (resume fast-path).** When `trial_id` is cheaply derivable without ingestion — manifest `nct_number`, manifest `trial_label`, a regex `NCT\d{8}` scan of the raw PDF text, or `sha256(paper_bytes)` ([REQ-05](#req-05-trial-metadata-extractor) precedence **minus** the LLM-extracted-NCT rung) — skip the whole entry **before `ingest_trial`** (unless `--force`) if **either**: (a) the entry **enumerates its `outcomes`** (so the full `(outcome, effect_of_interest)` pair set is known without the LLM) **and** every expected `(trial_id, outcome, effect_of_interest, model_sq, pipeline_version)` row is already present; **or** (b) a **`__TRIAL__` skip row** exists for the full sentinel key `(trial_id, "__TRIAL__", "__NA__", model_sq, pipeline_version)` (the trial was already judged ineligible — [REQ-16](#req-16-output-json-and-sqlite-writers)). Both checks are the same cheap `trial_id`-keyed DB lookup, and together they keep an interrupted-batch resume from re-paying the metadata + up-to-40-annotation calls for **either** already-completed **or** already-skipped trials. **Fall through** to the full path below when neither holds — e.g. `outcomes` defaults to `[primary]` (unknown until metadata extraction) and no skip row exists, or `trial_id` would need the LLM-extracted NCT (no manifest NCT/label **and** the regex finds none) — correctness wins over the fast-path in the ambiguous case. This is the same pre-ingest decision point the v0.2 refresh layer reuses to diff `inputs_hash` ([REQ-19](#req-19-python-api)).
- For each non-skipped entry: **`ingest_trial` once** ([REQ-19](#req-19-python-api)) → **`check_eligibility(ctx.trial_metadata, config)`** (below); on pass, **`assess_trial(ctx, …)`** — D1 once, then loop the entry's outcomes (outcome tier — D2/D3/D4/D5 per outcome), writing one record per trial-outcome. Ingestion happens exactly once, before the gate, so an ineligible trial is never ingested twice.
- **Resume-on-interrupt (per-outcome guard; not refresh-on-change):** the second-level guard for entries that _were_ ingested (fast-path didn't apply, e.g. a trial only partway done) — before assessing a trial-outcome, check the DB unique key `(trial_id, outcome, effect_of_interest, model_sq, pipeline_version)`; **skip** if already present unless `--force`. Because `trial_id` is deterministic ([REQ-05](#req-05-trial-metadata-extractor)) — including the NCT-less content-hash fallback — re-running a batch resumes by doing only missing/failed pairs even for trials with no registry metadata. **This is resume semantics, not refresh:** if a trial's _inputs_ change (a new supplement is published, the registry record updates) but the key is unchanged, the pair is **skipped, not re-assessed**. Refresh-on-change is a living-evidence concern deferred to v0.2 (see the limitation below); v0.1 ships only the runway for it: a non-keyed `inputs_hash` column on each row ([REQ-16](#req-16-output-json-and-sqlite-writers)) = a hash of `(paper_bytes, sorted supplement_bytes, ctgov_snapshot)`, where `ctgov_snapshot` is the CT.gov v2 `protocolSection.statusModule.lastUpdatePostDateStruct.date` (the registry record's last-update date — its stable per-fetch snapshot key; `None` when CT.gov was absent). It does not affect skip/resume behaviour today, but lets a future refresh layer detect stale rows with `WHERE inputs_hash != ?` and no schema migration.
...
- **Eligibility gate (deterministic; enforces IRPG scope) — a shared precondition, not a batch-only feature.** Factor it into a small helper `check_eligibility(trial_metadata, config) -> SkipRecord | None` (it takes `config` so the emitted `SkipRecord` can stamp `model_sq`/`pipeline_version` — the non-NULL key columns the skip row needs, [REQ-16](#req-16-output-json-and-sqlite-writers)), called on `ctx.trial_metadata` **after `ingest_trial`** ([REQ-19](#req-19-python-api), which performs the [REQ-05](#req-05-trial-metadata-extractor) metadata extraction) and **before** `assess_trial`, by **both** entry points: the batch runner (per entry) **and** `arbiter assess` ([REQ-18](#req-18-cli)) — so the single-trial path cannot bypass it and fabricate an out-of-scope assessment. Because ingestion already ran in `ingest_trial`, the gate reads metadata that exists, with no second ingestion. (`assess_trial` itself stays gate-free and "always computes" — [REQ-19](#req-19-python-api) — with the gate as a documented caller precondition; this keeps its `list[Assessment]` return type clean of skip handling.) The helper checks `trial_metadata.study_design`: if it is **not** `parallel_rct`, do **not** fabricate a five-domain assessment — emit a **`SkipRecord`** ([§5.4](#54-the-assessment-record-flat-one-per-trial-outcome)) and persist it via **`write_skip_record`** ([REQ-16](#req-16-output-json-and-sqlite-writers)): **one SQLite row per trial** (sentinel `outcome = "__TRIAL__"`, judgment columns `NULL`) carrying the design, the `study_design_basis`, `requires_human_review=True`, and a clear `errors` entry (`"ineligible study_design=<x>: <basis>"`), plus a `skip.json` for audit; then proceed (batch: next entry; assess: exit). No `Assessment` and no `data.json` are written for an ineligible trial. `unclear` is treated as **ineligible-but-flagged** (skipped, not assessed) so neither path ever emits a confident-looking RoB 2 score for an input ARBITER could not confirm is a parallel-group RCT. Because `unclear` is often an _extractor miss_ on a genuine parallel-group RCT, **`study_design` extraction reliability directly bounds batch coverage** — a quantity worth watching in the eval ([REQ-21](#req-21-evaluation-harness)); the conservative default is right for output consumed without a mandatory review gate (a fabricated assessment on a non-RCT is worse than a missing one), but a flaky classifier silently shrinks the assessed set. This is the analogue of auto-rob2's RCT-screening step, implemented as a deterministic gate over the existing aux call (no new LLM call, no LLM judgment).
...
- Re-running the same batch skips already-completed trial-outcome pairs; `--force` re-runs them. An entry whose outcomes are enumerated and **all** already complete is skipped **before `ingest_trial`** (no metadata/annotation calls re-paid); a partially-complete entry is ingested once and only its missing pairs are assessed.
...
Everything the dev smoke-test needs is committed under `eval/reference/` (private repo, so the trial PDFs ride along as fixtures; the `docs/rob2/` binaries are committed for the same reason). Only a future _public_ release strips third-party copyrighted material — nothing here is git-ignored for copyright:
...
> **Frozen thresholds — develop/report separation.** **No operational threshold is selected by maximising accuracy on any set whose accuracy is reported.** Every behaviour-governing knob ([REQ-01](#req-01-dependency-and-project-setup): retrieval/quote/outcome-match thresholds, token budgets, parsing heuristics) ships at a **conservative default frozen a priori** — set from the literature and accuracy-first intuition **before** the eval runs, not picked from a sweep. The ablation sweeps (recall@k, budget moves, etc.) are reported **purely as sensitivity/characterisation analyses** and never feed back into the shipped or reported configuration — this is why the recall@k ablation "does not decide the shipped retriever" ([REQ-03](#req-03-supplementary-material-ingestor)) and why the per-SQ/ablation results on ARBITER-Depth (n=15–25) carry **no train-on-test leak**: the set is far too small to carve a tuning split, so the only honest options are freeze-a-priori (chosen here) or tune on a dedicated non-reported dev slice (not used). The two genuine policy constants (`OVERALL_HIGH_SC_THRESHOLD`, the `SQRawAnswer` anti-abuse `max_length` ceilings — invariant 5, §2) are ADR/contract-owned ([REQ-07](#req-07-rob-2-algorithm-deterministic-judgments)/[§5.3](#53-trial--outcome-models)), reinforcing the same rule from the policy side.
...
| Step | Build                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Why                                                                                                                                                                                                                                              |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1    | REQ-01 setup + **§5.1–§5.5** data models (`models.py`) **incl. the `RunTrace` / `CallRecord` / `NodeSpan` model shells** (types only; collection/cost logic lands in step 11). **§5.6 graph state is NOT here** — `AssessmentRuntime`/`TrialContext` forward-reference `LLMClient` (step 7) and `SupplementIndex` (step 8), so §5.6 lands in step 9 with the graph.                                                                                                                                                                        | Everything imports from `config.py` / `models.py`; the trace **types** must exist now because steps 7 and 9 wire the optional `trace` seam against them                                                                                          |
...
| 9    | **§5.6 graph state** (`graph/state.py`: `TrialState`/`OutcomeState`/`DomainContext`/`AssessmentRuntime`/`TrialContext` — depends on `LLMClient`/`SupplementIndex` from steps 7/8), then **leaf nodes first, then orchestrate:** REQ-13 pre_d5 + REQ-14 context assembly + REQ-15 SQ worker (each mock-testable standalone), **then** REQ-12 two-tier graphs + `resolve_loop` + parallel domain branches. (D3 **text-only**; CONSORT vision node deferred to v0.2.) **Wire only the `trace` seam** in the builder against the step-1 shells | Core assessment — §5.6 state lands here because it needs steps 7/8 types; build and pin the leaf nodes with `MockLLMClient` before standing up the novel orchestration (cyclic `resolve_loop`, `Send` fan-out, parallel domain fan-in) over them |
...
- [ ] `arbiter batch` runs unattended; re-run **resumes** (fully-complete enumerated entries skipped **before** ingestion, no aux calls re-paid; partial entries ingested once, only missing pairs assessed); `--force` re-runs. (REQ-17)

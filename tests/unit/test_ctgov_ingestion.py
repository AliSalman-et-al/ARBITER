...
@pytest.mark.asyncio
async def test_fetch_ctgov_returns_verbatim_json(
    ...
    monkeypatch.setattr(
        "arbiter.ingestion.ctgov._make_transport", lambda: httpx.MockTransport(handler)
    )
...
@pytest.mark.asyncio
async def test_fetch_ctgov_returns_none_for_http_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def handler(_request: httpx.Request) -> httpx.Response:
...
@pytest.mark.asyncio
async def test_fetch_ctgov_returns_none_for_network_error(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def handler(_request: httpx.Request) -> httpx.Response:
...
@pytest.mark.asyncio
async def test_fetch_ctgov_returns_none_for_malformed_json(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def handler(_request: httpx.Request) -> httpx.Response:

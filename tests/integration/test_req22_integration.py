...
def _section_map() -> SectionMap:
    return SectionMap(
        ...
        page_boxes=[
            PageBox(
                boxclass="text", text=PAPER_TEXT[:120], bbox=(0, 0, 100, 100), page=0
            ),
            PageBox(
                boxclass="text", text=PAPER_TEXT[120:], bbox=(0, 110, 100, 200), page=0
            ),
        ],
...
def _config(
    tmp_path: Path, *, outcomes: list[str] | None = None, effect: str = "assignment"
) -> AssessmentConfig:
...
@pytest.mark.asyncio
async def test_full_trial_two_tier_reuse_writes_json_sqlite_and_report(
    tmp_path: Path,
) -> None:
    client = MockLLMClient(responses=_assignment_responses())
    config = _config(
        tmp_path, outcomes=["Overall Survival", "Progression-free Survival"]
    )
    ...
    assert (
        tmp_path / "out" / "trial-1" / "overall_survival__assignment" / "data.json"
    ).exists()
    assert (
        tmp_path / "out" / "trial-1" / "overall_survival__assignment" / "report.md"
    ).exists()
    with sqlite3.connect(config.db_path) as conn:
        rows = conn.execute(
            "SELECT outcome FROM arbiter_assessments ORDER BY outcome"
        ).fetchall()
    assert rows == [("Overall Survival",), ("Progression-free Survival",)]


@pytest.mark.asyncio
async def test_llm_failure_on_one_sq_completes_with_flagged_ni_and_errors(
    tmp_path: Path,
) -> None:
    client = MockLLMClient(
    ...
    assessment = (
        await assess_trial(
            _ctx(client), _config(tmp_path, outcomes=["Overall Survival"])
        )
    )[0]
...
async def test_quote_verification_failure_flags_answer(tmp_path: Path) -> None:
    client = MockLLMClient(
        responses=_assignment_responses(
            {"1.1|assignment": _raw("Y", "This quote is absent from the paper.")}
        )
    )
...
async def test_d2_branching_assignment_and_adhering_effects(tmp_path: Path) -> None:
    ...
    assignment_assessment = (
        await assess_trial(
            _ctx(assignment), _config(tmp_path / "a", outcomes=["Overall Survival"])
        )
    )[0]
    ...
    assignment_d2 = next(
        domain
        for domain in assignment_assessment.domain_judgments
        if domain.domain == "D2"
    )
    adhering_d2 = next(
        domain
        for domain in adhering_assessment.domain_judgments
        if domain.domain == "D2"
    )
    assert {answer.sq_id: answer.answer.value for answer in assignment_d2.sq_answers}[
        "2.4"
    ] == "NA"
    assert {answer.sq_id: answer.answer.value for answer in adhering_d2.sq_answers}[
        "2.7"
    ] == "NA"
...
async def test_trace_side_channel_and_trace_levels(tmp_path: Path) -> None:
    ...
    payload = json.loads(
        (config.output_dir / "trial-1" / "trace.json").read_text(encoding="utf-8")
    )
    data = json.loads(
        (
            config.output_dir / "trial-1" / "overall_survival__assignment" / "data.json"
        ).read_text()
    )
    ...
    await assess_trial(
        _ctx(
            MockLLMClient(responses=_assignment_responses(), trace=summary_trace),
            trace=summary_trace,
        ),
        summary_config,
    )
    summary_payload = json.loads(
        (summary_config.output_dir / "trial-1" / "trace.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    await assess_trial(
        _ctx(
            MockLLMClient(responses=_assignment_responses(), trace=off_trace),
            trace=off_trace,
        ),
        off_config,
    )
...
@pytest.mark.asyncio
async def test_full_qa_trace_bundle_covers_assess_run_and_is_tail_safe(
    tmp_path: Path,
) -> None:
    ...
    artifact_roots = {
        ref.split("/", 1)[0] for event in events for ref in event["artifact_refs"]
    }
...
@pytest.mark.asyncio
async def test_batch_idempotency_skips_second_run_and_force_reruns(

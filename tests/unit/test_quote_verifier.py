...
def test_verify_quote_exact_match() -> None:
    assert verify_quote(
        "Allocation was centrally concealed.", "Allocation was centrally concealed."
    )
...
def test_verify_quote_tolerates_common_ocr_artifacts() -> None:
    ...
    assert verify_quote(
        "The trial used stratified randomization with central allocation.", stream
    )
...
def test_locate_quote_page_returns_correct_zero_based_page(monkeypatch) -> None:
    ...
    assert (
        locate_quote_page("random allocation sequence was generated by computer", pages)
        == 1
    )


def test_locate_quote_page_returns_earlier_page_for_page_break_straddle(
    monkeypatch,
) -> None:
    monkeypatch.setenv("ARBITER_QUOTE_MIN_VERIFY_CHARS", "15")
    pages = [
    ...
    assert (
        locate_quote_page(
            "allocation was concealed until participants were enrolled", pages
        )
        == 3
    )


def test_locate_quote_page_resolves_quote_spanning_two_boxes_on_one_page(
    monkeypatch,
) -> None:
    ...
    assert (
        locate_quote_page("allocation sequence was concealed until enrolment", pages)
        == 2
    )
...
def test_locate_quote_page_returns_none_when_absent(monkeypatch) -> None:
    ...
    assert (
        locate_quote_page(
            "Allocation was concealed.", [box(0, "No matching methods text.")]
        )
        is None
    )
...
def test_resolve_quote_treats_absent_short_quote_as_unverified(monkeypatch) -> None:
    ...
    verified, page = resolve_quote(
        "short quote", "no matching text", [box(0, "no matching text")]
    )
...
def test_resolve_quote_source_verifies_unpaged_registry_short_quote(
    monkeypatch,
) -> None:
    monkeypatch.setenv("ARBITER_QUOTE_MIN_VERIFY_CHARS", "15")
